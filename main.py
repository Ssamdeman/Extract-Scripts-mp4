...
def print_verbose_output(analysis_results, filename):
    ...
    print(f"\n📈 Top {len(frequencies)} Most Used Meaningful Words:")
...
def process_and_analyze_file(file_path, model, json_path, transcript_path, is_text_file, verbose):
    """
...
def main():
    ...
    if not args.file:
        parser.error("the following arguments are required: file (unless using --llm-prompt)")
    ...
    # Detect file type
    is_text = detect_file_type(args.file)
    
    # Setup output paths
    json_path, transcript_path = setup_output_paths(args.file, args.output_dir)
    
    # Process file
    analysis_results = process_and_analyze_file(
        args.file, model, json_path, transcript_path,
        is_text, args.verbose
    )
    ...
    # Output results
    if args.verbose:
        print_verbose_output(analysis_results, os.path.basename(args.file))
    else:
        print_minimal_output(os.path.basename(args.file), json_path)
    
    # Show graph if requested
    if args.graph:
        display_frequency_graph(
            analysis_results['word_frequency'],
